...
export class StaticGetter implements VariableGetter {
  content : {[key: string] : ParamType};
  constructor(content : {[key: string] : ParamType}) {
    this.content = content;
  }
  getVariable(key: string) {
    if (!(key in this.content)) throw new NonExistingVariableMemoryError("read", key);
